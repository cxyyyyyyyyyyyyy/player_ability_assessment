...
(function () {
    ...
    function buildUI() {
        ...
        // 悬浮按钮
        floatBtn = el("button", "xl-float-btn", '<img src="' + AVATAR + '" alt="C小罗" onerror="this.style.background=\'#c9a45c\';this.remove()">');
        ...
        panel.innerHTML =
            ...
            '  <button class="xl-close" id="xlClose" title="收起">×</button>' +
            '</div>' +
            '<div class="xl-msgs" id="xlMsgs"></div>' +
            '<div class="xl-inputbar">' +
            '  <input id="xlInput" type="text" placeholder="问我点什么…（回车发送）" autocomplete="off">' +
            '  <button class="xl-send" id="xlSend">发送</button>' +
            '</div>';
        document.body.appendChild(panel);

        msgsBox = panel.querySelector("#xlMsgs");
        inputEl = panel.querySelector("#xlInput");
        sendBtn = panel.querySelector("#xlSend");
        var closeBtn = panel.querySelector("#xlClose");
    ...
    function send(raw) {
        ...
        else if (norm(text).indexOf(norm("你是谁")) >= 0 || norm(text).indexOf(norm("你是")) >= 0) chips = QUICK_QUESTIONS;
